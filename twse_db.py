...
def crawl_price(date):
    ...
    for item in data['data5']:
        ...
        prices.append([str_to_num(s, types[i])
...
def bulk_insert(fname, bulk_data):
    ...
    for d in bulk_data:
        ...
        cmd = 'INSERT OR REPLACE INTO daily_price VALUES({})'.format(
            ','.join(values))
        # print(cmd)
        c.execute(cmd)
    c.execute('COMMIT')
...
def update_db(date_from, date_to):
    print('更新資料: {} 到 {}'.format(date_from.strftime(
        '%Y-%m-%d'), date_to.strftime('%Y-%m-%d')))
    current = date_from
...
if __name__ == '__main__':
    ...
    print('資料庫日期: {} 到 {}'.format(db_from.strftime(
        '%Y-%m-%d'), db_to.strftime('%Y-%m-%d')))
